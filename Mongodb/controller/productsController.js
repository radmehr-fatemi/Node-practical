...
async function createProduct(req, res) {
    try {
        ...
        req.on("end", async () => {
            const product = { ...JSON.parse(body), createAt: new Date() }
            const created = await productsModel.create(product)
            res.writeHead(201, { "Content-Type": "Application/json" })
            res.write(JSON.stringify({ product, massage: "Created product" }))
            res.end()
        })
...
async function updateProduct(req, res) {
    try {
        ...
        req.on("end", async () => {
            ...
            res.write(JSON.stringify({ product, massage: "Updated the product" }))
            res.end()
        })
...
async function deleteProduct(req, res) {
    try {
        ...
        req.on("end", async () => {
            ...
            res.write(JSON.stringify({ massage: "Deleted the product" }))
            res.end()
        })
